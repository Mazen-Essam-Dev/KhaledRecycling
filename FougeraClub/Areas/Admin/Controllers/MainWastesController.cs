...
namespace KhaledTeamRecycling.Areas.Admin.Controllers
{
    [AdminAuthorize]
    [Area("Admin")]
    public class MainWastesController : Controller
    {
        private readonly IMainWasteService _mainWasteService;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public MainWastesController(IMainWasteService mainWasteService, IUnitOfWork unitOfWork, IMapper mapper)
        {
            _mainWasteService = mainWasteService;
        ...
        public async Task<IActionResult> AddEdit(int? id)
        {
            ...
            {
                return View(new MainWasteVM());
            }

            var entity = await _mainWasteService.GetByIdAsync(id.Value);
            if (entity == null)
            {
                return NotFound();
            }

            var vm = _mapper.Map<MainWasteVM>(entity);
            return View(vm);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> AddEdit(MainWasteVM model)
        {
            ...
            var entity = _mapper.Map<MainWaste>(model);

            if (model.Id == 0)
            {
                await _mainWasteService.AddAsync(entity);
                return RedirectToAction(nameof(Index));
            }

            await _mainWasteService.UpdateAsync(entity);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var hasRelated = await _mainWasteService.HasRelatedObjectsInDb(id);
            if (hasRelated)
            {
                TempData["Error"] = "Cannot delete this item because it has related SubWaste records.";
                return RedirectToAction(nameof(Index));
            }

            await _mainWasteService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        public async Task<IActionResult> createExcelReport_Download(string? searchTerm)
        {
            ...
            return File(
                ...
                $"MainWastes_{AppDubaiTime.Now:yyyyMMdd_HHmmss}.xlsx");
        }
    }
}
