...
using FougeraClub.Areas.Admin.ViewModels.LogHistory;
...
namespace FougeraClub.Areas.Admin.Controllers
{
    ...
    public class LogHistoryController : Controller
    {
        ...
        public async Task<IActionResult> Timeline(string? selectedUserId, string? selectedMemberEmail, DateTime? dateFrom, DateTime? dateTo, int page = 1, int pageSize = 40)
        {
            ...
            var memResource = @Resource1.member??" ";
            var allMembersName = await _unitOfWork.Members.Table.Select(
            ...
            var Logs_query = from log in _unitOfWork.RequestLogs.Table
                             ...
                             join member in _unitOfWork.Members.Table
                             on log.UserId equals member.Email into memberGroup
                             from member in memberGroup.DefaultIfEmpty() // Left Join members by email
                             select new LogsVM
                             {
                                 Id = log.Id,
                                 UserId = log.UserId,
                                 ...
                                 ? (user != null ? user.FullNameAr : (member != null ? member.FullNameAr : string.Empty))
                                 : (user != null ? user.FullNameEn : (member != null ? member.FullNameEn : string.Empty)),
                                 UserFullNameAr = user != null ? user.FullNameAr : member != null ? member.FullNameAr : null,
                                 UserFullNameEn = user != null ? user.FullNameEn : member != null ? member.FullNameEn : null,
                                 UserKind = user != null ? "SystemUser" : (member != null ? "Member" : null)
                             };
            ...
            ViewBag.allMembersName = SelectListHelper.BindSelectList(allMembersName, null, "Email", "FullNameAr", "FullNameEn").Distinct();
        ...
        public async Task<IActionResult> Index(string? searchTerm, string? selectedUserId, string? selectedMemberEmail, DateTime? dateFrom, DateTime? dateTo, int page = 1, int pageSize = 40)
        {
            ...
            var Logs_query = from log in _unitOfWork.RequestLogs.Table
                             ...
                             on log.UserId equals user.Id into userGroup
                             from user in userGroup.DefaultIfEmpty() // Left Join
                                                                     //where (!dateFrom.HasValue || log.RequestTime.Date >= dateFrom.Value.Date.AtMidnight()) &&(!dateFrom.HasValue || log.RequestTime.Date >= dateFrom.Value.Date.AtMidnight())
                             select new LogsVM
                             {
                                 Id = log.Id,
                                 UserId = log.UserId,
                                 ...
                                 LogTarget = log.LogTarget,
                                 RequestTime = log.RequestTime,
                                 UserFullName = lang == "ar" ? user.FullNameAr : user.FullNameEn,
                                 UserFullNameAr = user.FullNameAr,
                                 UserFullNameEn = user.FullNameEn,
                                 UserKind = user != null ? "SystemUser" : "Member"
                             };
        ...
        public async Task<IActionResult> createExcelReport_Download(string? searchTerm, string? selectedUserId, string? selectedMemberEmail, DateTime? dateFrom, DateTime? dateTo, int page = 1, int pageSize = 500)
        {
            ...
            var allMembersName2 = SelectListHelper.BindSelectList(allMembersName, null, "Email", "FullNameAr", "FullNameEn").Distinct();
            ...
            try
            {
                ...
                if (allData_list != null || allData_list?.Count() > 0)
                {
                    ...
                    var excelDataDTO = allData_list.Select((log, index) =>
                    {
                        ...
                        {
                            userFullName = Resource1.member + " - " + allMembersName2
                                ?.Where(mem => mem.Value.ToString() == log.UserId)
                                .FirstOrDefault()?.Text;
                        }
                        if (userFullName == Resource1.member + " - ")
                        {
